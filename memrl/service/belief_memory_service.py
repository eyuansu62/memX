...
class BeliefMemoryService(MemoryService):
    ...
    def resolve_memories(
        ...
        prompt = (
            "You are a state compiler for a task-solving agent in a household environment. "
            "Given retrieved memories from past experiences on similar tasks, "
            "compile them into an actionable state view.\n"
            f"{belief_instruction}\n"
            "CRITICAL RULES:\n"
            "- PRESERVE exact action names, object names, and location names "
            "(e.g., 'go to fridge 1', 'take apple 1 from countertop 2', "
            "'heat apple 1 with microwave 1'). Do NOT paraphrase actions.\n"
            "- The agent needs to copy-paste these actions. Abstract descriptions "
            "like 'find the apple and heat it' are USELESS.\n"
            "- If multiple memories show different locations for the same object, "
            "list all locations with their success counts.\n\n"
            "Output ONLY the compiled state in this format:\n\n"
            "## Best Action Sequence\n"
            "(The highest-confidence step-by-step action sequence from successful "
            "memories. Use exact action syntax.)\n\n"
            "## Alternative Locations\n"
            "(Other locations where target objects were found, with success counts.)\n\n"
            "## Known Pitfalls\n"
            "(Specific actions or locations that failed. Use exact names.)\n\n"
    ...
    def generic_summary(
        ...
        prompt = (
            ...
            "IMPORTANT: Preserve exact action names, object names, and "
            "location names (e.g., 'go to fridge 1', 'take apple 1'). "
            "Do NOT paraphrase into abstract descriptions.\n\n"
            "Organize your summary into these sections:\n\n"
            "## Useful Action Sequence\n"
            "(Step-by-step actions that worked, using exact syntax)\n\n"
            "## Potential Pitfalls\n"
            "(Specific actions or locations that failed)\n\n"
            "## Open Questions\n"
