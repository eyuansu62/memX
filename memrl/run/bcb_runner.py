...
# ── Standalone helper for parallel evaluation ─────────────────────────────────
...
def _evaluate_one_standalone(
    ...
    """Evaluate a single BCB solution (called from ThreadPool)."""
    if not test_code:
    ...
    stat, details, err, hard_timed_out = run_untrusted_check_with_hard_timeout(
        ...
        min_time_limit=1.0, gt_time_limit=float(eval_timeout_s),
        hard_timeout_s=float(hard_timeout_s), bcb_repo=bcb_repo,
    )
    ...
    def _run_phase(
        ...
        *,
        ...
        for batch_start in range(0, total, BATCH):
            ...
            # --- 1. Retrieve memories (serial, fast) ---
            ...
            for i, task_id in enumerate(batch_ids):
                ...
                if update_memory:
                    ...
                    if sel_ids or r_trace:
                        retrieval_for_traj = {"selected_ids": list(sel_ids), "num_retrieved": len(sel_ids), "trace": r_trace}
                    pending_task_descriptions.append(prompt)
                    pending_trajectories.append(self._trajectory_from_raw_or_fallback(
                        raw_response=raw_response, prompt=prompt, code=code, eval_res=eval_res, retrieval=retrieval_for_traj))
            ...
            self._tb_add_scalar(f"bcb/{phase}/pass_at_1", (pass_count / float(global_idx)) if global_idx else 0.0, step=step)
            if self.retrieve_k > 0:
